...
class Music():
    ...
    def play_music(self, audio, music_name):
        ...
        new_filename = music_name + '.wav'

        with open(filename, 'wb') as file:
            file.write(music_data.content)

        subprocess.Popen(['ffmpeg', '-i', filename, new_filename]).wait()

        audio.play_audio(new_filename)

        subprocess.Popen(['rm', filename, new_filename]).wait()
